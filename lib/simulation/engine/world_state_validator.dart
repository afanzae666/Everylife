...
class WorldStateValidator {
  ...
  Result<void> validate(WorldState state) {
    ...
    for (final entry in invalidStats.entries) {
      if (!_isValidStat(entry.value)) {
        return Failure(
          'World state is invalid: player '
          '${entry.key} must be between 0 and 100.',
        );
      }
    }
